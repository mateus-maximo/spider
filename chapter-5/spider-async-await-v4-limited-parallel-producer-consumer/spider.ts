...
async function spiderTask(url: URL, nesting: number, queue: TaskQueue): Promise<SpiderResult> {
  ...
  const { content, downloaded } = await queue.runTask(async () => {
    try {
      return {
...
async function spiderLinks(url: URL, content: string, nesting: number, queue: TaskQueue): Promise<void> {
  if (nesting === 0) {
    return
  }
  ...
  const promises = links.map((link) => {
    ...
    return spiderTask(nextUrl, nesting - 1, queue)
  })
